...
test("project negotiation requires project id and absolute physical path", () => {
  assert.equal(
    isNDXProjectConfigureMessage({
      type: NDX_PROJECT_CONFIGURE,
      projectId: "project-1",
      projectPath: "/mnt/f/dev/ndx2"
    }),
    true
  );
  assert.equal(
...
test("session attach accepts existing session paths including legacy relative paths", () => {
  assert.equal(
    isNDXSessionAttachMessage({
