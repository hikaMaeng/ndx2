...
export async function runToolProcess(
  ...
  const fillTemplate = async (template: string): Promise<string> => {
    if (isToolRuntimeArgName(template)) {
      return resolveToolRuntimeArg(template, systemArgContext);
    }
  ...
  const guardedResult = options.denyToolResultEffects && result.effects?.length
    ? {
      ...result,
      status: "failed" as const,
      success: false,
      output: `Tool result effects are not allowed in this session: ${tool.name}`,
      error: `Tool result effects are not allowed in this session: ${tool.name}`,
      effects: undefined
    }
    : result;
  await options.observer?.onToolFinished?.(guardedResult);
