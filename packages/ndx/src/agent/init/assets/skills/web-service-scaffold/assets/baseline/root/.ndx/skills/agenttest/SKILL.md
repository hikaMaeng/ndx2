---
name: agenttest
description: Use when creating or running NDX-executed, filesystem-backed agent tests from JSON suites plus proof assets. Supports category-parallel subagents, one-test-at-a-time execution, result submission, status polling, and final report generation without calling an external LLM API.
---

# Agenttest

Use this skill when the user asks for AI-friendly tests, NDX-executed tests,
...
- NDX executes only the current test returned by the runner.
...
```text
test/20260512/014021_service-health/health-endpoint_probe.ts
test/20260512/014021_service-health/health-endpoint_expected.txt
```
...
```json
{
  "id": "suite-id",
  "title": "Suite title",
  "dependencies": { "service": "apps/example" },
  "items": {
    "category": [
      {
        "id": "test-id",
...
4. Write steps in arrange, act, assert order. Each step must be runnable by
   NDX without inventing hidden files or commands.
...
For example, an HTTP smoke test can start the scaffold service on a temporary
port, request `/health`, and assert that the JSON body contains the generated
service name rather than only checking for any successful response.
...
```bash
yarn exec tsc .ndx/skills/agenttest/scripts/agenttest.ts --target ES2022 --module NodeNext --moduleResolution NodeNext --outDir .ndx/skills/agenttest/dist
```

Run the compiled CLI:

```bash
node .ndx/skills/agenttest/dist/agenttest.js validate test/YYYYMMDD/HHMMSS_suite-name.json
node .ndx/skills/agenttest/dist/agenttest.js start test/YYYYMMDD/HHMMSS_suite-name.json
node .ndx/skills/agenttest/dist/agenttest.js next <workerUuid>
node .ndx/skills/agenttest/dist/agenttest.js submit <workerUuid> <result.json>
node .ndx/skills/agenttest/dist/agenttest.js status <runUuid>
node .ndx/skills/agenttest/dist/agenttest.js finalize <runUuid>
```
...
12. Poll `status` from the main NDX session.
