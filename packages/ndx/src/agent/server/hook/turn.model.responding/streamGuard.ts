...
const MAX_REASONING_ONLY_MS = 120_000;
...
export const modelResponseStreamGuardHook: NDXHookCodeExecutor = {
  ...
  run(context): NDXHookEffect {
    ...
    if (context.modelResponse.elapsedMs > MAX_REASONING_ONLY_MS) {
      streamGuardState.delete(key);
      return interruptEffect(`model response reasoning exceeded ${MAX_REASONING_ONLY_MS}ms before producing output.`);
    }
