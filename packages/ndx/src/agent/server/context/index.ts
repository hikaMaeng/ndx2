...
export async function buildContext(sessionMetadata: SessionMetadata): Promise<BuiltContext> {
  ...
  const userSections = [
    ...(await buildUserInstructions(sessionMetadata)),
    buildEnvironmentContext(sessionMetadata),
  ];

  return {
    developer: developerSections.filter((section) => section.length > 0).join("\n\n"),
    user: userSections.filter((section) => section.length > 0).join("\n\n"),
  };
}
