...
import type { NDXResolvedTool, NDXToolDefinitionFile, NDXToolRegistryOptions, NDXToolRuntimeArgName, NDXToolScope } from "./types.js";
...
export function resolveBuiltinToolRoot(): string {
  return path.join(moduleDirectory, "basetools");
}
...
function isKnownToolRuntimeTemplate(value: string): value is NDXToolRuntimeArgName {
  return (NDX_TOOL_RUNTIME_ARG_NAMES as readonly string[]).includes(value);
}
