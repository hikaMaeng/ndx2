...
| Client request queue domain state, per-session queue mutation, queued-item projection, and queue drain policy inputs | `packages/ndx/src/agent/requestQue` | The app socket server may register a queue registry, persist queued attachments through session helpers, broadcast queue snapshots, and call the turn loop when a queued item is popped. Queue data structure and mutation rules stay out of `apps/ndx`. |
