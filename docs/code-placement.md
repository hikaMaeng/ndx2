...
| Agent turn loop orchestration | `packages/ndx/src/agent/turnloop` | Owns user input persistence, context reconstruction, model streaming, assistant persistence, compact continuation, and turn lifecycle events. `turnloop/after-loop` finalizes one turn and runs the final `turn.end` hook; post-response request effects are interpreted by the root turn-loop runner after the current turn is closed and launched from a later macrotask. App socket code only adapts protocol events to transport messages and starts the turn-loop API for accepted input or an idle queued session. |
