...
| Session search projection and `session_history` query policy | `packages/ndx/src/agent/session`, `packages/ndx/src/agent/tool/base/session_history` | `sessionsearch` is derived from `sessiondata` and remains agent session authority. Keep row projection/query SQL with session persistence; keep the function-tool adapter with the `session_history` base tool. |
| Tool-specific special message handling such as `cot_work` agent calls | `packages/ndx/src/agent/tool/base/<tool>` for concrete parsing/validation/action, `packages/ndx/src/agent/tool/execute/agentcall` for envelope parsing and routing | The agent-call executor only recognizes the generic transport envelope and dispatches by name. Tool-specific rules stay with the base tool folder. |
