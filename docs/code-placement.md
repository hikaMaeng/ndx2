...
| Tokenizer-backed text token counting | `packages/ndx/src/common/tokenizer` | Keep the signature model-neutral: text plus optional model or encoding hints. Agent context usage, compaction, and prompt-budget code may depend on this utility; tokenizer code must not import agent, webclient, app, hook, or turn-loop modules. |
| `.ndx/settings.json` raw document schema, path/IO helpers, provider/model/embedding row normalization, runtime-settings projection, and websearch settings merge rules | `packages/ndx/src/common/settings` | This is a Node/server-only common contract shared by agent runtime, process-tool launch, and webclient settings persistence. Agent and webclient modules may expose domain-specific wrappers, but they must not own duplicate settings document types or parse `settings.json` directly. |
...
| Agent turn loop orchestration | `packages/ndx/src/agent/turnloop` | Owns user input persistence, context reconstruction, model streaming, assistant persistence, request-time compaction, and turn lifecycle events. `turnloop/after-loop` finalizes a normal completed turn and runs the final `turn.end` hook. `turnloop/base/compact` interprets existing request-received hook `compact` effects before the new user row is persisted; compact is not a separate hook event and does not end an already completed turn. Post-response request effects are interpreted by the root turn-loop runner after the current turn is closed and launched from a later macrotask. App socket code only adapts protocol events to transport messages and starts the turn-loop API for accepted input or an idle queued session. |
| Client request queue domain state, per-session queue mutation, queued-item projection, and queue bridge contracts | `packages/ndx/src/agent/requestQue`; post-response claim policy in `packages/ndx/src/agent/hook/base/requestQueue` | The app socket server may register a queue registry, persist queued attachments through session helpers, broadcast queue snapshots, provide a queue edit bridge to tools, and provide a separate queue consumer bridge to the base hook/root launcher. Queue data structure and mutation rules stay out of `apps/ndx`; claiming a queued request after a finalized turn is a base `turn.end` hook that returns a typed `turnEndRequest` effect. |
| Model-facing request-queue planning tools such as `turnplan` | `packages/ndx/src/agent/tool/base/<tool>` for schema and queue-edit policy, `packages/ndx/src/agent/requestQue` for queue mutation types, `apps/ndx/src/server/agent` for connected-client queue broadcasts | `turnplan` is a producer/editor for the existing session request queue, not a consumer, scheduler, hook, or turn-loop phase. The turn loop passes only the queue edit bridge into tool execution; queue consumption stays with the base hook/root launcher path. |
| Final model request message composition, sessiondata-to-model projection, one-request attachment append, and model-visible suppression policies | `packages/ndx/src/agent/turnloop/model-call/finalMessages` | Session modules own durable rows and text helpers only. `index.ts` owns `prepareFinalModelRequestMessagesForCall` and the non-exported final-message and row-projection policy order; `messages/` and `rows/` own only the individual policy implementations before `turn.model.request` prefix drift auditing. |
...
| Session search projection, exact sessiondata recall, and `session_history` query policy | `packages/ndx/src/agent/session`, `packages/ndx/src/agent/tool/base/session_history` | `sessionsearch` is derived from `sessiondata` and remains agent session authority. Exact recall by `dataid` or `dataid` range reads `sessiondata` directly. Keep row projection/query/recall SQL with session persistence; keep the function-tool adapter, mode schema, model-facing system skill, and turn-end system hook policy with the `session_history` base tool. The hook event registration stays in `packages/ndx/src/agent/hook`. |
...
| Tool executor agent-call messages such as `cot_work` and right-sidebar `session.sidebar_item` | `packages/ndx/src/agent/tool/base/<tool>` for concrete payload creation, `packages/ndx/src/agent/tool/execute/agentcall` for envelope parsing, validation, and routing | Tools emit structured intermediate results but do not own socket transport. Generic routing and common sidebar validation stay in the executor; durable tool-specific rules stay with the base tool folder. `cot_work` live-plan validation, timing policy, and context-prepared reminder hook policy belong under `packages/ndx/src/agent/tool/base/cot_work`. The final `cot_work` completion write is a base `turn.end` hook under `packages/ndx/src/agent/hook/base/cotWorkCompletion` that calls those tool-owned timing/sidebar policies before the request-queue hook can trigger the next turn. |
...
| Webclient server-only settings/state persistence helpers | `packages/ndx/src/webclient/server/settings` | Owns webclient settings API persistence operations, provider/model catalog mutations, embedding settings updates, model patch drafting, runtime/tool/hook/websearch form updates, and regression tests. It uses `packages/ndx/src/common/settings` for the raw `settings.json` schema, IO, and shared normalization; it must not define a parallel settings document contract. May help create agent session requests, but does not own turns, tools, inference, or context reconstruction. |
| Webclient-only session favorites/pinned-session persistence | `packages/ndx/src/webclient/server/session-favorite` for table and persistence helpers, `apps/ndx/src/server/web/webclient/session-favorites` for Express routes | This is a webclient backend feature, not agent execution authority. It may reference agent session rows for display and FK cleanup, but must not enter the session socket server, turn loop, tools, inference, or context reconstruction. |
...
* Agent execution authority belongs to `packages/ndx/src/agent`. Webclient, settings, CLI, and other future clients may request or display agent work, but must not own the agent loop, tool calls, inference, or context reconstruction.
...
5. Is it UI composition or layout? Put webclient composition in `apps/ndx/src/webclient_front`, settings composition in `apps/ndx/src/webclient_front/settings`, and document-site composition in `apps/ndx/src/documents_front`; put reusable UI-facing domain helpers in the matching package front surface.
