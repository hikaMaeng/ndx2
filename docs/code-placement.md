...
| Final model request message composition, sessiondata-to-model projection, one-request attachment append, and model-visible suppression policies | `packages/ndx/src/agent/turnloop/model-call/finalMessages` | Session modules own durable rows and text helpers only. The final-message policy pipeline is the single place that turns rows into provider-visible messages before `turn.model.request` prefix drift auditing. |
