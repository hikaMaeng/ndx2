...
`userid`, `projectid`, project path, and model config. Model selection is
...
`NDX_ROOT` map to `/ndx`. The container workspace root and user home are fixed
...
* if the selected skill is not already present in session history, the hook
  records a `loadSkill`-shaped assistant tool result containing the full
  `<skill>...</skill>` body;
