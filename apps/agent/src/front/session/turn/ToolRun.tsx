...
import type { TurnToolState } from "./types";
...
export function ToolRun({ tool }: { tool: TurnToolState }) {
  ...
  return (
    <details className="rounded-md border border-zinc-800 bg-zinc-950/70 px-3 py-2" data-testid="turn-tool-run">
      ...
      <div className="mt-3 grid gap-2 text-xs text-zinc-400">
        {tool.callId ? <p><span className="text-zinc-500">call</span> {tool.callId}</p> : null}
        {tool.args ? <pre className="max-h-36 overflow-auto rounded bg-black/30 p-2 text-zinc-300">{JSON.stringify(tool.args, null, 2)}</pre> : null}
        {tool.progress.length > 0 ? (
          <ol className="grid gap-1" aria-label={`${tool.tool} progress`}>
            {tool.progress.map((item) => <li key={item.id} className="rounded bg-zinc-900 px-2 py-1">{item.text}</li>)}
          </ol>
        ) : <p className="text-zinc-600">No progress events yet.</p>}
        {tool.result ? (
          <section className="grid gap-1" aria-label={`${tool.tool} result`}>
            <p className="text-zinc-500">result</p>
            <pre className="max-h-72 overflow-auto rounded bg-black/40 p-2 text-zinc-300">{toolResultText(tool.result)}</pre>
          </section>
        ) : null}
      </div>
    </details>
