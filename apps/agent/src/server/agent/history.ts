import { NDX_TURN_EVENT, type NDXSessionEventMessage, type NDXSessionIterationSummary, type NDXSessionTurnSummary } from "ndx/agent/common";
import { listSessionData, type NDXDatabase, type NDXSessionDataRow } from "ndx/agent/server";
import { sessionDataToSessionEvent, type NDXAgentWebSessionData } from "ndx/agent/web";
...
export async function buildSessionHistorySummary(database: NDXDatabase, sessionid: string) {
  const turns = await listSessionHistoryTurns(database, sessionid);
  return {
...
function toHistoryTurn(inputEvent: NDXSessionEventMessage, events: NDXSessionEventMessage[]): SessionHistoryTurn {
  ...
  return {
    ...
    summary: {
      ...
      status: finalEvent ? "completed" : interrupted ? "interrupted" : "running",
