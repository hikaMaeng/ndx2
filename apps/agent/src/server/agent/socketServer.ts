...
import { NDX_CLIENT_ID_QUERY_PARAM, isNDXClientId } from "ndx/agent/common";
...
export function attachSessionSocketServer(server: http.Server, options: AttachSessionSocketServerOptions) {
  ...
  server.on("upgrade", (request, socket, head) => {
    ...
    if (connectedClients.has(clientid)) {
      options.logger?.warn("agent.socket.upgrade.rejected", { clientid, reason: "duplicate_clientid" });
