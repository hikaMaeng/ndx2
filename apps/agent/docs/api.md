...
* `GET /api/agent/web-projects` lists persisted web-client projects.
* `POST /api/agent/web-projects` registers a project path after resolving or creating its `project` table row by `target + path`.
* `GET /api/agent/projects/:projectid/sessions` lists sessions for a project and user.
* `POST /api/agent/projects/:projectid/sessions` creates a PostgreSQL-backed session.
...
4. Client sends one `project.configure` with `projectId` and absolute `projectPath`.
