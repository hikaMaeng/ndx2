...
Turn loop가 매 iteration마다 PostgreSQL에서 history를 다시 읽는 이유는 하나다. 실행 중 memory가 편하더라도 authoritative state가 되면 crash recovery와 prefix-cache 계약이 깨진다. `packages/ndx/src/agent/turnloop/base/context/index.ts`는 developer, user prelude, history, inline attachment 순서를 고정하고, `runAgentTurn`은 그 순서를 반복해서 사용한다.
