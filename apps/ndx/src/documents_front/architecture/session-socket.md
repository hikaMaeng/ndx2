...
Socket server가 session state machine이 되면 HTTP client, future CLI, settings UI가 각자 다른 agent semantics를 갖게 된다. 그래서 socket은 connection, grants, event translation, pending browser interaction만 담당한다. 세션 생성, history, tool execution, context reconstruction은 package domain이 계속 소유한다.
