...
NDX는 pgvector, Chromium, Docker CLI, Playwright runtime, Node runtime을 포함하는 복합 런타임이다. npm 릴리스 빌드는 `npm/Dockerfile`로 이 런타임과 app artifact를 하나의 final image에 담고, npm 사용자는 `ghcr.io/hikamaeng/ndx2-agent:<version>` 하나만 실행한다. 로컬 개발용 base archive는 npm 경로에 사용하지 않는다. 사용자가 로컬 Node/Yarn/Turbo 빌드 체인을 모두 맞추게 하는 대신, npm 런처는 Docker stack을 만들고 browser URL만 알려준다.
