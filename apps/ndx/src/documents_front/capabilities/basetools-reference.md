...
`turnplan`은 현재 turn 안의 작업 체크리스트가 아니라 session request queue를
관리한다. `plan`은 목표와 작업 요청 목록을 받아 작업 요청 사이에 성찰
요청을, 마지막에는 목표 달성 요약 요청을 자동 삽입한다. `list`는 큐를
조회하고, `add`, `update`, `delete`, `clear`는 남은 큐 항목을 조정한다.
남은 큐가 목표에 맞는지 분석하는 책임은 도구가 아니라 성찰 turn의 모델에
있다.

`turnplan`으로 삽입된 항목은 특별한 runtime이 아니라 일반 queued work로
실행된다. 성찰 turn 역시 일반 turn이므로 `$turnplan` 스킬, 파일/도구/세션
기록을 사용하고, 남은 큐가 목표에 맞지 않으면 다시 `turnplan`으로
조정한다.
