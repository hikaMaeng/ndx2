...
| `apps/ndx/src/server/web/webclient/settings` | settings route orchestration and HTTP translation. |
...
| `apps/ndx/src/documents_front/getting-started` | install and first-session Markdown documents. |
| `apps/ndx/src/documents_front/operations` | Docker, monorepo, testing Markdown documents. |
| `apps/ndx/src/documents_front/reference` | source map, inventory, long-run documentation plan. |
| `apps/ndx/src/documents_front/resources` | document-local static resources and future images. |
...
| `packages/ndx/src/agent/context/reasoningDiscipline` | reasoning discipline instruction policy. |
...
이 인벤토리는 사람이 읽는 색인이면서 코드 감사의 입력이다. 새 디렉터리가 추가되면 `apps/ndx/src/documents_front/audit.mjs`가 실패해야 하고, 문서 작성자는 해당 경로의 책임과 관련 문서를 추가해야 한다.
