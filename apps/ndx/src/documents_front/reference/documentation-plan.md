...
| 14 | session server authority | webclient/settings/CLI가 agent loop를 소유하지 않음을 설명. |
...
| 19.8 | settings surface boundary | `apps/ndx/src/webclient_front/settings`, `apps/ndx/src/server/web/webclient/settings`, `packages/ndx/src/webclient/server/settings` 경계 설명. |
...
| 22 | function tools | askUserQuestion, session_history 설명. |
...
| 27.4 | rewriter marker examples | `[[rewriter]]` marker와 `session_history` 입력/출력 예시 설명. |
