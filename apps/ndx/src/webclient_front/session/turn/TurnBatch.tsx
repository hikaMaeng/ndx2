...
export function TurnBatch({ batch, onToggle }: { batch: TurnBatchState; onToggle?: (batch: TurnBatchState, open: boolean, userInitiated: boolean) => void }) {
  ...
  return (
    <details className="min-w-0 overflow-hidden rounded-md border border-zinc-800 bg-zinc-900/70 p-3" open={!batch.collapsed} data-testid="turn-iteration" onToggle={(event) => {
      ...
      <summary
        className="flex cursor-pointer list-none items-center justify-between gap-3 text-sm font-medium text-zinc-200"
        onPointerDown={() => {
      ...
      <div className="mt-3 grid min-w-0 gap-2">
        ...
        {batch.modelEvents.length > 0 ? (
          <ol className="grid min-w-0 gap-1 text-xs text-zinc-500" aria-label={`Iteration ${batch.iteration} model request events`}>
