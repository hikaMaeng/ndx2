...
export const ChatFolderCard = React.memo(function ChatFolderCard({
  ...
  return (
    <li className="min-w-0">
      <div className={folder.folderid === activeFolderId ? "w-full min-w-0 overflow-hidden rounded-md border border-emerald-700 bg-emerald-950/30 px-2 py-1 text-left" : "w-full min-w-0 overflow-hidden rounded-md border border-zinc-800 bg-zinc-900/60 px-2 py-1 text-left hover:bg-zinc-900"} aria-busy={pending}>
        <div className="flex min-w-0 items-center gap-1">
          <Button type="button" size={null} className="flex min-w-0 flex-1 items-center gap-2 text-left" onClick={() => onSelectFolder(folder)}>
        ...
        {sessions.length > 0 ? (
          <ul className="mt-1 grid min-w-0 gap-1" aria-label="채팅 세션 목록">
