...
export const ChatSessionRow = React.memo(function ChatSessionRow({
  ...
  return (
    <li className="min-w-0" aria-busy={pending}>
      <div className={session.chatsessionid === activeSessionId ? "flex w-full min-w-0 items-center gap-1 overflow-hidden rounded border border-emerald-700 bg-emerald-950/50 px-1 py-0.5 text-xs text-emerald-200" : "flex w-full min-w-0 items-center gap-1 overflow-hidden rounded border border-zinc-800 bg-zinc-950 px-1 py-0.5 text-xs text-zinc-400 hover:bg-zinc-900"}>
        <Button type="button" size={null} disabled={pending} className="block min-w-0 flex-1 overflow-hidden px-1 text-left disabled:pointer-events-none disabled:opacity-50" onClick={() => onSelectSession(session)}>
