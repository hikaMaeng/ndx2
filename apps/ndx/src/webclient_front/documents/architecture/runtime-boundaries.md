...
Admin surface는 운영과 계정/설정 관리를 위한 별도 UI가 될 수 있지만, agent turn을 대신 실행하지 않는다. 설정 변경이 agent behavior에 영향을 주더라도 실행 권한은 `packages/ndx/src/agent`가 가진다.

## Socket server 경계

`apps/ndx/src/server/agent`는 HTTP server에 WebSocket을 붙이고, connection token, history replay, downstream event broadcast를 다룬다. 하지만 최종적으로 turn을 시작하고 데이터를 쓰는 함수는 agent package의 domain API다.

이 경계가 필요한 이유는 간단하다. transport가 늘어나도 agent semantics가 하나여야 한다. Web, future CLI, admin, remote client가 생겨도 같은 PostgreSQL session truth와 같은 turn loop를 사용해야 한다.
