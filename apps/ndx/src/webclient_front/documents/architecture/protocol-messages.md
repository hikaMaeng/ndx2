...
| `account.selection.required` | 서버가 account 선택을 요구. |
| `account.select` | client가 userid를 선택. |
| `account.selected` | 서버가 account selection을 확정. |

로그인 flow가 아직 명시되지 않은 경우 server는 기본 계정 `ndev`를 선택하는 product contract를 따른다.
...
| 메시지 | 방향 | 설명 |
| --- | --- | --- |
| `session.create` | client -> server | project/model/user 정보를 바탕으로 새 session 생성. |
| `session.attach` | client -> server | connection token으로 기존 session event 권한 획득. |
