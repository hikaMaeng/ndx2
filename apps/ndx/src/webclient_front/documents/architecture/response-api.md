...
| fallback input | provider가 structured input을 거부하면 text serialization input을 시도한다. |
...
일부 provider는 Responses API 형식의 structured array input을 완전히 지원하지 않는다. 400 응답에 `Invalid type for 'input'`이 포함되면 같은 messages를 text block으로 직렬화해 다시 보낸다. 이 fallback은 호환성을 위한 장치지만, prompt ordering 문서와 테스트에서 반드시 고려해야 한다.
