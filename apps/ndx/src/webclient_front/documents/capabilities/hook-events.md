...
| `turn.model.request` | 최종 model request 직전 | `prefixDrifts`, `diagnostics` (prefix drift 감사) |
...
| 이벤트 | system hook |
| --- | --- |
| request received | thinking marker, skill marker, context limit |
| context prepared | cot work reminder, inline input images, context limit |
| model request | prefix drift audit |
| model responding | stream guard |
| tool called | (system hook 없음) |
| tool results collected | loop detection |
| turn end | input session search, assistant session search, turn context usage |
