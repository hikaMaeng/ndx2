...
| `apps/ndx/src/server/web/admin` | admin API route surface. |
| `apps/ndx/src/webclient_front` | browser app shell, session UI, menu, chat, modal, docs. |
| `apps/ndx/src/admin_front` | admin UI shell. |
...
| `packages/ndx/src/admin` | admin common/server/front contracts. |
...
| `pgvector/Dockerfile.pgvector` | Korean FTS and pgvector base image. |
...
현재 1차 강제 지점은 `apps/ndx/src/webclient_front/documents/coverage.json`과
`apps/ndx/src/webclient_front/documents/audit.mjs`다. coverage source path는 실제
