...
* `GET /api/agent/projects/:projectName/sessions` lists sessions for a project.
...
2. Client sends one `project.configure` with `projectName`.
3. Server sends `session.ready`.

After `session.ready`, the socket accepts `session.input` and `session.interrupt`
messages for sessions in the negotiated project. The server
