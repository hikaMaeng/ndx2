...
* One service: `bash .codex/skills/web-deploy-docker/scripts/deploy.sh <service-or-app-path>`
* All services, only when explicitly requested: `bash .codex/skills/web-deploy-docker/scripts/deploy.sh --all`

After reading this file, the next tool call must be that bundled deploy command
when `.codex/skills/web-deploy-docker/scripts/deploy.sh` exists.

Do not search for this skill, inspect `.codex`, list files, read package files,
